...
### Repositórios do GitHub — snapshot estático + cache
Em vez de o navegador de cada visitante chamar a API do GitHub diretamente (sujeita ao limite de 60 requisições/hora por IP, sem autenticação), o site lê um arquivo estático (`public/github-repos.json`), gerado por `scripts/fetch-github-repos.mjs`:

- O workflow do GitHub Actions roda **a cada hora** (`schedule: cron`), busca os repositórios usando o `GITHUB_TOKEN` interno do Actions (limite de 5000 req/hora — folgado) e reimplanta o site com o snapshot atualizado.
- No cliente, `useGithubRepos` ainda mantém um cache em `localStorage` por cima do snapshot, pra exibição instantânea e como proteção extra caso o arquivo estático falhe ao carregar.
- Pra atualizar o snapshot manualmente em dev: `npm run fetch:github`.
